...
> ## ⚠️ Este arquivo NÃO é a identidade que o assistente carrega
>
> Descreve a **arquitetura do sistema** — propósito, público, stack. Nenhum runtime o lê:
> procurou-se por carregador na auditoria de 13/09 e não existe.
>
> A identidade conversacional do Rayzen — voz, relação com Marcelo, postura — vive em
> **[`infra/hermes/SOUL.md`](../../infra/hermes/SOUL.md)**, montada `:ro` no perfil do Hermes, ao
> lado do `config.yaml`. Que ela é de fato carregada foi **verificado** em 14/09, com uma regra
> observável no arquivo e a resposta do modelo obedecendo.
>
> Dois arquivos com o mesmo nome significando coisas diferentes era o problema; os nomes dos
> diretórios agora os separam. **Ao mudar como o assistente se comporta, o arquivo é o outro.**
>
> O texto abaixo tem duas afirmações que envelheceram e ficam como registro, não como descrição
> do estado atual: *"Claude Code é o motor de inteligência interno"* deixou de valer para todos
> os caminhos (a auditoria de 13/09 já apontava), e a execução conversacional passa hoje por
> Hermes + modelo substituível.
...
| ADR-006 | Docker Compose no servidor local (Ubuntu) | Ativo |
