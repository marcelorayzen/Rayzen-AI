...
import { useGuardian, gateRiskLevel, type GuardianReport, type PendingGate } from '../hooks/useGuardian'
...
function timeLeft(expiresAt: string | null): string {
  ...
  return min < 60 ? `~${min}min pra expirar` : `~${Math.round(min / 60)}h pra expirar`
}
...
function PendingGateCard({
  ...
  const level = gateRiskLevel(gate)

  return (
    <div
      className="rounded-xl p-4 flex items-center justify-between gap-4 flex-wrap"
      style={{ background: RISK_BG[level] ?? RISK_BG.medium, border: `1px solid ${RISK_COLOR[level] ?? RISK_COLOR.medium}40` }}
    >
      <div className="flex flex-col gap-1 min-w-0">
        <p className="text-sm font-medium" style={{ color: 'var(--hud-text)' }}>{gate.description}</p>
        <span className="text-xs" style={{ color: 'var(--hud-dim)' }}>
          {new Date(gate.createdAt).toLocaleString('pt-BR')}
          {gate.expiresAt && ` · ${timeLeft(gate.expiresAt)}`}
        </span>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          style={{ background: RISK_COLOR[level] ?? RISK_COLOR.medium, color: '#fff' }}
        >
          aprovar
        </button>
      </div>
...
export default function GuardianPage() {
  ...
  const {
    latest, history, loading, error, reload, override,
    pendingGates, gateActionId, approveGate, rejectGate,
  } = useGuardian(projectId)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--hud-bg)', color: 'var(--hud-text)' }}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        {/* pending approval gates — não aparecem na tela de missões (gates sem missionId) */}
        {projectId && !loading && pendingGates.length > 0 && (
          <section>
            <h2 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--hud-dim)' }}>
              Aprovações pendentes ({pendingGates.length})
            </h2>
            <div className="flex flex-col gap-2">
              {pendingGates.map((g) => (
                <PendingGateCard
                  key={g.id}
                  gate={g}
                  busy={gateActionId === g.id}
                  onApprove={() => approveGate(g.id)}
                  onReject={() => rejectGate(g.id)}
                />
              ))}
            </div>
          </section>
