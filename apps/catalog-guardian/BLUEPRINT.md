...
1. ✅ **Identidade** — resolvido no roadmap item 5 (`README.md § Roadmap`), escopo deliberado: gate de domínio inalterado (já era real, via Domains nativos do OMD) + clearance de PII via Role/Policy reais (`matchAnyTag('PII.Sensitive')`), não uma policy engine genérica — sem herança de role via Team, sem outras condition functions do OMD (`isOwner`, `matchTeam`, `hasAnyRole` etc.). `effect: 'deny'` passou a ser respeitado no item 7 (antes só `allow` era considerado — um `deny` mais específico sobre um `allow` amplo era ignorado). Suficiente pro golden dataset atual; expandir escopo só se um cliente real precisar de regra que não caiba nisso — ver README § Backlog.
