...
1. ✅ **Identidade** — resolvido no roadmap item 5 (`README.md § Roadmap`), escopo deliberado: gate de domínio inalterado (já era real, via Domains nativos do OMD) + clearance de PII via Role/Policy reais (`matchAnyTag('PII.Sensitive')`), não uma policy engine genérica. `effect: 'deny'` passou a ser respeitado no item 7 (antes só `allow` era considerado — um `deny` mais específico sobre um `allow` amplo era ignorado). Backlog de identidade fechado em 2026-07-30: `evaluateCondition()` ganhou `isOwner()`/`hasAnyRole()`/`inAnyTeam()`/`hasDomain()` (`matchTeam()` reconhecida mas fail-closed de propósito, exigiria modelar anexação de policy a uma entidade — não modelado), e roles herdadas via `team.defaultRoles` (subindo `team.parents` recursivamente) agora entram na descoberta de policy, não só nas condições — ver README § Roadmap item 5.
...
4. **PII no `UnityCatalogAdapter` (Fase 6) é uma convenção nossa (`properties.pii == "true"`), não um padrão do produto** — diferente do OMD, que tem Tags/Classification nativas (`PII.Sensitive`). Um cliente real de Unity Catalog não vai ter essa property populada por padrão; onboarding real exigiria documentar/migrar essa convenção antes de confiar na detecção de PII contra a UC. Ver README § Backlog.
