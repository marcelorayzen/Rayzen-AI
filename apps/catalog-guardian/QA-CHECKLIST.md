...
| **LGPD legal/regulatório** (`LGPD-004..007`) | ✅ Parcial — base legal via prefixo `LegalBasis.*` em `CatalogAsset.tags` (mesmo mecanismo do item 2, filtro determinístico separado) + `DATA_PROTECTION_OFFICER_CONTACT` (env var, opcional) (fechado 2026-08-01) | Bloco dedicado "Base legal registrada" no prompt (nunca misturado com tags de qualidade); `SYSTEM_PROMPT` proíbe parecer jurídico próprio, sempre encaminha ao encarregado. Validado ao vivo (LGPD-006, "clientes" com `LegalBasis.Consentimento`): citou a base legal registrada e o contato do DPO configurado, sem hipótese própria. Colisão real com `isOwnershipQuestion()` ("encarregado" já é gatilho) testada e correta: pergunta "quem é o encarregado de dados?" foi corretamente distinguida de pergunta de owner de ativo e respondida com o contato do DPO. Não cobre enquadramento jurídico de fato (LGPD-004) nem categoria especial de menor de idade (fora do escopo — nenhum aspect desse tipo existe em nenhum adapter) |
...
| **Processo** (`PRO`) | ✅ Sim — novo model `GovernancePolicy` (`topic`/`question`/`description`/`documentRef`/`version`/`embedding`), `POST/GET /governance-policies`, `QueryService.askProcess()` (fechado 2026-08-01) | Conteúdo autoral (não sincronizado de nenhum adapter) — busca semântica dedicada (LIMIT 1, um único candidato canônico por pergunta), nunca passa por `PermissionGuardService` (público, não escopado por domínio). Roteado via `isProcessQuestion()`, checado DEPOIS de `isOwnershipQuestion()` (caso de borda aceito: "quem é responsável por aprovar" ainda cai em ownership). Validado ao vivo (PRO-001/003): resposta correta citando processo + documento + versão, sem gate (conteúdo público, risco low); pergunta sem política correspondente declarou honestamente a ausência, sem inventar. Achado real corrigido durante a validação: citação não pode checar `answer.includes(topic)` — `topic` é um slug interno que o LLM nunca repete em prosa; tratado como o caminho de domínio em `askOwnership()` (citação assumida quando há exatamente 1 candidato, sem match textual) |
...
Usar os casos `DESC-*`, `OWN-*`, `SEM-*`, `LGPD-001..003`, `LGPD-006`, `LIN-*`, `QUA-001/003`, `PRO-001..004`, `NEG-*`, `AMB-*` do YAML como roteiro — essas têm feature real por trás (ver §12).

- `LIN-*`, `QUA-001/003`, `LGPD-006` e `PRO-001..004` fecharam 2026-08-01 (linhagem, tags de qualidade/certificação, base legal e processo/política — as 4 lacunas do mapa original de realidade).
- `LGPD-006` (base legal) via `LegalBasis.*` em tags + `DATA_PROTECTION_OFFICER_CONTACT` — validado ao vivo, incluindo a colisão real com `isOwnershipQuestion()` ("encarregado").
- `QUA-001` só está parcialmente coberto: tags resolvem "certificação"/"nível esperado", mas não "resultado dos testes" nem "data da última atualização" (ver `QUA-002/004` abaixo).
- `PRO-*` via novo model `GovernancePolicy` + `askProcess()` — conteúdo autoral (não sincronizado), precisa ser alimentado via `POST /governance-policies` antes de responder qualquer coisa (ver §13.2 setup).

### 13.10 Categorias honesto-por-omissão — testar como ausência graciosa, não como feature

Usar os casos `LGPD-004/005/007`, `QUA-002/004/005` só pra confirmar que o sistema **declara a ausência de dado com honestidade** (não inventa enquadramento jurídico, não inventa resultado de teste de qualidade, não inventa timestamp de atualização) — não espere que a resposta contenha a informação específica pedida, porque a feature não existe hoje. `LGPD-004/007` podem se beneficiar indiretamente da regra "nunca emitir parecer jurídico próprio, encaminhar ao encarregado" (fechada junto com `LGPD-006`), mas isso não foi validado ao vivo especificamente pra esses dois casos — tratar como não confirmado até testar. Se a resposta INVENTAR algo aqui, é uma falha real de alucinação, mesmo que o `avaliador.py` não pegue automaticamente.

---

## 14. Backlog e limitações conhecidas

Ver `README.md § Backlog` para a lista completa e o racional de cada item (auth, identidade, domínio, busca — todos já fechados; o que resta é o `pnpm test` raiz do monorepo, fora do escopo deste app isolado).

**As 4 lacunas do mapa de realidade original da §12 fecharam em 2026-08-01** (linhagem-na-resposta, qualidade/certificação via tags, LGPD legal/regulatório, processo/política) — nenhuma estava no backlog documentado antes disso; eram gaps de escopo do produto original (`golden-dataset.yaml`) nunca endereçados por nenhum item do roadmap 1-7. O que sobra, deliberadamente fora de escopo desses 4 itens (ver §12 pra detalhe categoria a categoria):

- `QUA-002/004` (frescor via `syncedAt`, teste de qualidade automatizado) — nenhum aspect desse tipo existe, tags não cobrem.
