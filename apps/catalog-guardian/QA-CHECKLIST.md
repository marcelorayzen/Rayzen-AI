...
Usar os casos `DESC-*`, `OWN-*`, `SEM-*`, `LGPD-001..003`, `LGPD-006`, `LIN-*`, `QUA-001/003`, `NEG-*`, `AMB-*` do YAML como roteiro — essas têm feature real por trás (ver §12).

- `LIN-*` e `QUA-001/003` fecharam 2026-08-01 (linhagem e tags de qualidade/certificação — eram honesto-por-omissão antes).
- `LGPD-006` (base legal) fechou 2026-08-01 via `LegalBasis.*` em tags + `DATA_PROTECTION_OFFICER_CONTACT` — validado ao vivo, incluindo a colisão real com `isOwnershipQuestion()` ("encarregado").
