...
| **Qualidade** (`QUA`) | ❌ **Não existe** — sem campos de certificação/teste de qualidade/última execução no `CatalogAsset`. `syncedAt` existe mas **nunca é exposto no contexto do LLM** | Mesma situação: honesto-por-omissão, não feature real |
