 ...
 *
 * ⚠️ **CÓPIA da V1** — o original é
 * `apps/api/src/modules/project-state/event-derived-text.const.ts`. Existe porque a
 * V1 limpa na leitura pelo `ProjectStateService.serialize()`, mas o `V1BridgeService`
 * daqui lê a linha crua do Prisma e nunca passa por ele: sem esta cópia o mesmo
 * ProjectState responde duas coisas diferentes conforme quem pergunta.
 *
 * A duplicação é consciente, como a de `memory-ranking`: `packages/types` **não é
 * compilado**, e importar valor de lá derruba o container. O drift é barrado por
 * `__tests__/event-derived-text.spec.ts`, que lê o arquivo da V1 como texto e falha
 * se divergir. **Ao mexer aqui, mexa lá também.**
 *
 * ── Por que existe ──────────────────────────────────────────────────────────────
