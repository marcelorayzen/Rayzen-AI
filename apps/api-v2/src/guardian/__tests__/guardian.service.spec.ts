...
function buildDeps(overrides: {
  ...
  // Alimenta o sinal semTesteRodado — o Guardian pergunta à V1 há quanto tempo
  // uma execução de teste cobriu este projeto.
  const bridge = { horasDesdeUltimoTeste: jest.fn().mockResolvedValue(0.5) }
  const svc = new GuardianService(
    prisma as never, gapDetector as never, riskScorer as never, approvalGates as never, bridge as never,
  )
  return { svc, prisma, gapDetector, riskScorer, approvalGates, bridge, fakeReport }
}
