...
export class GuardianService {
  ...
  async analyze(dto: GuardianAnalyzeDto, jwtExpiresInDays?: number): Promise<GuardianReport> {
    ...
    // Falha na consulta vira `undefined`, não `null`: undefined faz o scorer pular
    // o sinal, enquanto null significaria "nunca rodou teste" e pontuaria. Um erro
    // de leitura não pode virar acusação.
    const horasDesdeUltimoTeste = await this.bridge
      .horasDesdeUltimoTeste(dto.projectId)
      .catch(() => undefined)
