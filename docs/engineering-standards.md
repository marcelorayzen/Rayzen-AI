...
- **Separação de papéis do Agent** — Agent `desktop` roda no PC de trabalho (screenshot, clipboard, testes locais); Agent `server` roda no servidor que hospeda a stack (logs, Docker, restarts); ambos compartilham polling e whitelist; `jarvis:restart_api` sempre roteia para `server`
